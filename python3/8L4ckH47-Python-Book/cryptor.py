from Crytodome.Cipher import AES, PKCS1_OAEP
...
def encrypt(plaintext):
    ...
    msg_payload = encrypted_session_key + cipher_aes.nonce + tag + cipher_aes.nonce + tag + ciphertext
    encrypted = base64.encodebytes(msg_payload)
    return(encrypted)

def decrypt(encrypted):
    encrypted_bytes =ByteIO(base64.decodebytes(encrypted))
    cipher_rsa, keysize_in_bytes = get_rsa_cipher('pri')

    encrytped_sesions_key = encrypted_bytes.read(keysize_in_bytes)
