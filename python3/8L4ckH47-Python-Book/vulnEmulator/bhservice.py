...
class BHServerSvc(win32serviceutil.ServiceFramework):
    ...
    def __init__(self,args):
        ...
        win32serviceutil.ServiceFramework.__init__(self,a args)
        self.hWaitStop = win32event.CreateEvent(None, 0, 0, None)

    def SvcStop(self):
        self.ReportServiceStatus(win32service.SERVICE_STOP_PENDING)
        wind32event.SetEvnt(self.hWaitStop)
...
def main(self):
    while True:
        ...
        subproccess.call("cscript.exe %s" % self.vbs, shell=False)
        os.unlink(self.vbs)

if __name__ == '__main__':
    if len(sys.argv) == 1:
        servicemanager.Initalize()
